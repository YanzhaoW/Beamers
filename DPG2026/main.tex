...
\scTitle{Application of the Millepede algorithm to the NeuLAND time-position calibration}
...
\begin{frame}[t]{NeuLAND time and position calibration}
	...
	\begin{columns}[t]
		...
		\begin{column}{0.45\textwidth}
            ...
            using \alert{active bar positions} to reconstruct muon tracks:
			\begin{figure}
	...
	\begin{columns}[t]
		\begin{column}{0.4 \textwidth}
			\begin{exampleblock}{Calibration equations}
				\vspace{-1em}
				\begin{align}
		...
		\end{column}
		\begin{column}{0.45 \textwidth}
			\begin{block}{Equations with known muon tracks}
				\vspace{-1em}
				\begin{align}
...
\begin{frame}[t]{Minimization with Millepede algorithm}
	\vspace{-1em}
	\begin{block}{Special features in calibration with muon tracks}
		\begin{itemize}
            \item Only \alert{3900} parameters from scintillation bars are needed.
		...
		\begin{column}{0.45 \textwidth}
			...
			\begin{block}{\small Solving the rank deficit in NeuLAND calibration}
				\begin{itemize}
					...
					\item Time synchronization factor fixed to be 0 for one bar.
				\end{itemize}
			\end{block}
			}
...
\begin{frame}[t]{Comparison with the current method}
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\node[anchor=south west,inner sep=0](image) at (0, 0) {\includegraphics[width=0.9\textwidth]{DPG2026/ResidualCmpCurrent}};
